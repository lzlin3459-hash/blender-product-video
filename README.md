...
留下来的，是一套能继续改的工程和几个值得重复使用的做法：**先看样板，按重叠内容拼图，先跑通完整样片，再精修可见部分。**
...
把完整样片提前，把修改范围说清楚，把收尾变成流程的一部分。

- 产品制作与建筑制作分开触发，普通讨论和文档修改不会加载制作流程。
- 沿用已确认的镜头与声音，不为每个低风险步骤增加确认。
- 只重渲染受影响的镜头，复用未变化部分并保留来源；工程哈希检查仍然保留。
- 保留规格、完整解码及视听检查；没有实际听过，就不宣称通过听感验收。

校园和建筑项目请使用独立的 [Blender Architecture Video](https://github.com/lzlin3459-hash/blender-architecture-video)。
...
品牌和标签内容不要改。先检查外观样板，再完成低清全片和高清交付。
```
